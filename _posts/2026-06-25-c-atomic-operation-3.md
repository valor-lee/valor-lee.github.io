...
이때 가장 다루기 쉬운 경우는 atomic 연산의 대상이 정렬되어 있고 하나의 cache line 안에 들어 있는 경우다.

```text
정렬된 단일 cache line 접근
...
+------------------------------------------------+
              ^
              |
              +-- lock xadd가 이 cache line의 쓰기 권한을 얻고
                  read-modify-write를 atomic하게 수행한다.
```

예를 들어 접근 주소가 정렬되어 있지 않거나 하나의 atomic 접근이 cache line 경계를 걸치면 CPU가 한 cache line만 배타적으로 확보해서 처리하기 어려울 수 있다.

```text
cache line 경계를 걸친 접근

Memory
+---------------- cache line #10 ----------------+
| ... | ... | part A of atomic object             |
+------------------------------------------------+
                         ^
                         |
                         | 하나의 atomic 객체가
                         ...
                         v
+---------------- cache line #11 ----------------+
| part B of atomic object | ... | ...            |
+------------------------------------------------+
```

그런 경우에는 단일 cache line에 대한 일반적인 fast path보다 비용이 커질 수 있다.

구현에 따라 더 넓은 범위의 bus lock, microcode 처리, 여러 cache line에 대한 조정, 또는 compiler/runtime의 보조 경로가 필요할 수 있기 때문이다.
...
|  cache line X: counter=10 | |  cache line X: counter=10 |
+---------------------------+ +---------------------------+
```
...
위 과정을 그림으로 줄이면 다음처럼 볼 수 있다.

```text
1. 둘 다 같은 cache line을 읽은 상태

Core A cache                         Core B cache
+------------------+                 +------------------+
| line X: value 10 |                 | line X: value 10 |
+------------------+                 +------------------+


2. Core A가 atomic update를 하려고 line X의 쓰기 권한을 얻음

Core A cache                         Core B cache
+------------------+                 +------------------+
| line X: value 10 |  invalidate --> | line X: invalid  |
| writable         |                 |                  |
+------------------+                 +------------------+
...
Core A cache                         Core B cache
+------------------+                 +------------------+
| line X: value 11 |                 | line X: invalid  |
| writable         |                 |                  |
+------------------+                 +------------------+


4. Core B가 다시 읽거나 쓰려면 최신 line X를 가져와야 함

Core A cache                         Core B cache
+------------------+   transfer -->  +------------------+
| line X: value 11 |                 | line X: value 11 |
+------------------+                 +------------------+
```
...
```text
하나의 cache line 안에 서로 다른 counter가 함께 배치된 경우
...
+------------------------------------------------------------+
        ^               ^
        |               |
        |               +-- Thread B가 second만 변경
        |
        +-- Thread A가 first만 변경

논리적으로는 서로 다른 변수지만,
CPU cache coherence 입장에서는 같은 cache line X를 두 thread가 번갈아 쓰는 상황이다.
```
